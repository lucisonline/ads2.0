...
/*
  ...
  Phase 1 (0–30%):   Hand peeks from right, "Design strategy presents" drops down from top
  Phase 2 (30–60%):  Cursors visible, hand drifts left, subtitle rises from below
...
export default function IntroHero() {
  ...
  const handX = useTransform(scrollYProgress, [0, 0.33, 0.66, 1], ['15%', '5%', '-5%', '-10%'])
  const handY = useTransform(scrollYProgress, [0, 0.66, 1], ['-5%', '-12%', '-19%'])
  ...
  // "Advertising in the age of agents" — rises up from below as hand approaches cursors
  const t2Bottom = useTransform(scrollYProgress, [0.05, 0.30], ['-10%', '12%'])

  return (
    <section className="intro-hero" ref={ref}>
      <div className="intro-hero__sticky">
        {CURSORS.map((cursor, i) => (
          <AnimatedCursor key={i} cursor={cursor} progress={scrollYProgress} index={i} />
        ))}

        <motion.div
          className="intro-hero__hand"
